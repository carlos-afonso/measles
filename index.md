...
To help understand the geographic distribution of the measles outbreak, the visualizations below highlight the locations of the affected neighborhoods on the NYC map, with colors representing the number of cases.
...
The visualization below shows the same two images from the animated GIF above but now in a static form, side-by-side. (To better see the images below when using a phone, turn to landscape mode or zoom in.)
...
The graph below shows the NYC measles outbreak evolution over time, by comparing the number of **new** measles cases per month. Note that April 2019 is very likely to get more cases in the next data update.
